...
function initVirtualPanel() {
    ...
    if (btnContainer) {
        let btnHtml = '';
        for (let i = 0; i <= 5; i++) {
            btnHtml += `<div class="board-btn off" id="k${i}" data-index="${i}"></div>`;
        }
    ...
    if (buzzerContainer) {
        buzzerContainer.innerHTML = '<div class="board-buzzer off" id="buzzer54">🔊</div>';
    }
...
class StudentDashboard {
    ...
    initPanels() {
        ...
        if (buzzerPanel) {
            ...
            buzzer.className = 'board-buzzer off';
            buzzer.id = 'buzzer54';
    ...
    initCustomCaptureModal() {
        ...
        segmentNames.forEach(seg => {
            ...
            label.innerHTML = `<input type="checkbox" value="SEG_${seg}"> SEG_${seg}`;
            digitContainer.appendChild(label);
        });
        for (let i = 0; i < 8; i++) {
            const label = document.createElement('label');
            label.className = 'channel-label';
            label.innerHTML = `<input type="checkbox" value="DIG${i}"> DIG${i}`;
            digitContainer.appendChild(label);
        }

        btnPanelContainer.innerHTML = '';
        for (let i = 0; i < 6; i++) {
    ...
    clearWaveform() {
        ...
        const btnPanel = document.getElementById('btn-container');
        btnPanel?.querySelectorAll('.board-btn').forEach(btn => {
            btn.classList.remove('on');
            btn.classList.add('off');
        });

        const buzzerEl = document.getElementById('buzzer54');
        if (buzzerEl) {
    ...
    mapWaveformsToUI(waveforms, pinMapping) {
        ...
        for (let index = 0; index < waveforms.length; index++) {
            ...
            else if (pinName.startsWith('DIG')) {
                const digIndex = parseInt(pinName.replace(/\D/g, ''), 10);
                if (!isNaN(digIndex)) {
                    const digitEl = document.querySelector(`.digit[data-index="${digIndex}"]`);
                    if (digitEl) {
                        digitEl.classList.toggle('on', isOn);
                        digitEl.classList.toggle('off', !isOn);
                        elementFound = true;
                    }
                }
            }
            ...
            else if (pinName === 'BUZZER' || pinName === 'BEEP') {
                const buzzerEl = document.getElementById('buzzer54');
                if (buzzerEl) {
